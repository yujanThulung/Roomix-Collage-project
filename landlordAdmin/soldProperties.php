...
<body style="margin-top:-1.5rem">
  ...
  <div class="container">
    ...
    <main>
      ...
      $filtered_property_query = "SELECT * FROM property WHERE sold_status = 0 AND user_id = " . $_SESSION['id']; //Here i add condition to filter properties based on logged-in user's user_id 
      ...
      <div class="pagination">
        ...
        $total_records_query = "SELECT COUNT(*) AS total FROM  property WHERE  sold_status= 0 AND user_id = {$_SESSION['id']}";
        ...
        if ($page > 1) {
          echo "<a href='soldProperties.php?page=" . ($page - 1) . "'>&laquo; Previous</a>";
        }

        // Display pagination links
        for ($i = 1; $i <= $total_pages; $i++) {
          echo "<a " . ($i == $page ? "class='active'" : "") . " href='soldProperties.php?page=$i'>$i</a>";
        }

        // Display "Next" button
        if ($page < $total_pages) {
          echo "<a href='soldProperties.php?page=" . ($page + 1) . "'>Next &raquo;</a>";
        }

        // Display "Last" button
        echo "<a href='soldProperties.php?page=$total_pages'>Last &raquo;&raquo;</a>";
        ?>
      </div>
